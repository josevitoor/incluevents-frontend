...
import { Drawer, Form, Input, DatePicker, Col, Row, Select, Steps, Button , message } from 'antd';
...
const EventosForm = ({ isVisible, onClose }) => {
    const optionsUf = ["AC", "AL", "AP", "AM", "BA", "CE", "DF", "ES", "GO", "MA", "MT", "MS", "MG", "PA", "PB", "PR", "PE", "PI", "RJ", "RN", "RS", "RO", "RR", "SC", "SP", "SE", "TO"];
    ...
    const handleSubmitForm = async () => {
        ...
        try {
            ...
            const data = {
                ...
                local: formatarEndereco(form.getFieldValue('rua'),
    ...
    const fetchAddress = async (cep) => {
        try {
          ...
          form.setFieldsValue({ uf, cidade: localidade, bairro: bairro, rua: logradouro });
        } catch (error) {
          message.error('Erro ao buscar endereço. Verifique o CEP e tente novamente.');
        } finally {
          setLoading(false);
        }
    };
    ...
    const getCityListByState = async (stateCode) => {
        try {
          const response = await axios.get(`https://servicodados.ibge.gov.br/api/v1/localidades/estados/${stateCode}/municipios`, {
          });
    ...
    return (
        ...
        footer={
            <Row gutter={16} justify="end"  className="footer-drawer">
                ...
                { currentStep < 1 && (
                    <Button type="primary" onClick={nextStep} className="footer-drawer-button">
                        Próximo
                    </Button>
                )}
                {currentStep === 1 && (
                    <Button type="primary" onClick={handleSubmitForm} className="footer-drawer-button">
                        Enviar
                    </Button>
                )}
            </Row>
            ...
            <Form form={form} onFinish={handleSubmitForm} layout="vertical" name="userForm">
                {currentStep === 0 && (
                    <Col>
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={8}>
                                <Form.Item
                                    name="data_evento"
                                    label="Data"
                                    rules={[{ required: true, message: 'Por favor insira o período do evento!' }]}
                                    >
                                        <RangePicker
                                            format="DD/MM/YYYY"
                                            showTime={false}
                        ...
                        >
                            <TextArea showCount maxLength={3000} placeholder="Descreva seu Evento" />
                        </Form.Item>
                        <Form.Item
                            name="categorias"
                            label="Categorias"
                            rules={[{ required: true, message: 'Por favor insira as categorias do evento!' }]}
                        >
                        ...
                        >
                            <Search addonBefore="https://" placeholder="Link do Ingresso" allowClear />
                        </Form.Item>
                        <Form.Item
                            name="capa_evento"
                            label="Capa do Evento"
                            rules={[{ required: true, message: 'Por favor insira o link da capa do evento!' }]}
                        >
                            <Search addonBefore="https://" placeholder="Link da Imagem" allowClear />
                        </Form.Item>
                    </Col>
                )}
                        ...
                        <Row gutter={16}>
                            ...
                            <Col span={8}>
                                <Form.Item
                                    name="uf"
                                    label="UF"
                                    rules={[{ required: true, message: 'Por favor insira a UF do evento!' }]}
                                >
                                    <Select placeholder="Selecione a UF" onChange={handleUFChange} value={selectedState}>
                        ...
                        <Row gutter={16}>
                            <Col span={12}>
                                <Form.Item
                                    name="cidade"
                                    label="Cidade"
                                    rules={[{ required: true, message: 'Por favor insira a cidade do evento!' }]}
                                >
                            ...
                            <Col span={12}>
                                <Form.Item
                                    name="bairro"
                                    label="Bairro"
                                    rules={[{ required: true, message: 'Por favor insira o bairro do evento!' }]}
                                >
                                    <Input />
                                </Form.Item>
                            </Col>
                        ...
                        <Row gutter={16}>
                            <Col span={16}>
                                <Form.Item
                                    name="rua"
                                    label="Rua"
                                    rules={[{ required: true, message: 'Por favor insira a rua do evento!' }]}
                                >
                                    <Input />
                                </Form.Item>
                            </Col>
                            <Col span={8}>
                                <Form.Item
                                    name="numero"
                                    label="Número"
                                    rules={[{ required: true, message: 'Por favor insira o número do evento!' }]}
                                >
                                    <Input />
                                </Form.Item>
                            </Col>
